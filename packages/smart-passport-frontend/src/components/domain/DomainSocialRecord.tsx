...
export function DomainSocialRecord(props: DomainSocialRecordProps) {
  return (
    <div className="flex items-center">
      <div className={'mr-2 flex-shrink-0 ' + (!props.displayName && !props.identity ? 'opacity-50' : '')}>
        <img src={props.icon} className="rounded-full" style={{ height: 24 }}></img>
      </div>

      <div className={'mr-3 truncate ' + (!props.displayName && !props.identity ? 'opacity-50' : '')}>{props.displayName || props.identity || "Not Set"}</div>
