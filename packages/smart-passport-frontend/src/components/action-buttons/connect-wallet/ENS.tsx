...
function ENSButtonUnstyled(props: any) {
  ...
  const handleSignMsg = useCallback(async () => {
    ...
    try {
      ...
      await attest('ens', walletAddress)
    ...
    <div {...props}>
      <div className="flex justify-between items-center mt-8">
        <div className="flex gap-2 w-64">
          <div>
            <img src="/images/ens.png" className="w-10 h-10" alt="" />
          </div>
          <div>
            <div className="flex gap-2 items-center">
              <div className="font-medium text-[#101828]">ENS</div>
              {walletState && (
                <div>
                  <svg
      ...
      <StyleProvider hashPriority="high">
        <Modal
          ...
          onOk={() => handleSignMsg()}
        >
          <div className="mb-2 text-black">Please enter your ENS domains</div>
          <Input
            value={walletAddress}
            onChange={(e) => setWalletAddress(e.target.value)}
            placeholder="optidomains.eth"
          />
        </Modal>
      </StyleProvider>
    </div>
...
export default styled(ENSButtonUnstyled)`
  .ant-menu {
