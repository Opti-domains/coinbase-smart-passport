...
function CoinbaseVerificationButtonUnstyled(props: any) {
    ...
    <div {...props}>
      <div className="flex justify-between items-center mt-8">
        <div className="flex gap-2 w-64">
          <div>
            <img src="/images/cb.svg" className="w-10 h-10 rounded-full" alt="" />
          </div>
          <div>
            <div className="flex gap-2 items-center">
              <div className="font-medium text-[#101828]">Coinbase Verification</div>
              {walletState && (
                <div>
                  <svg
            ...
            <div className="mt-1 text-[#667085] text-xs">
              {walletState
                ? `Linked from ${walletState.displayName.substring(0, 16)}...`
      ...
      <StyleProvider hashPriority="high">
        <Modal
          title={"Link Coinbase Verification"}
          open={walletSelectorModalOpen}
          confirmLoading={submitting}
          onCancel={() => setWalletSelectorModalOpen(false)}
          onOk={() => handleSignMsg()}
        >
          <div className="mb-2 text-black">Please enter your wallet that is verified on Coinbase Verification</div>
          <Input
            value={walletAddress}
            onChange={(e) => setWalletAddress(e.target.value)}
            placeholder="0x0123...abcd"
          />
        </Modal>
      </StyleProvider>
    </div>
...
export default styled(CoinbaseVerificationButtonUnstyled)`
  .ant-menu {
